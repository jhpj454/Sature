  ...
  try {
    ...
    const data = await withTenantClientFromRequest(request, async (client) => {
      const result = await client.query(
        `
          SELECT
            id, agency_id, policy_id, limit_type, limit_amount, applies_to,
  ...
  try {
    ...
    const created = await withTenantClientFromRequest(request, async (client, session) => {
      ...
      const insertRes = await client.query(
        `
          INSERT INTO policy_limits (
            agency_id, policy_id, limit_type, limit_amount, applies_to
          ) VALUES ($1, $2, $3, $4, $5)
          RETURNING *
        `,
        [
      ...
      await emitEvent(client, {
        agencyId: session.agency_id,
        eventType: "policy_limit.created",
        entityType: "policy_limit",
        entityId: row.id,
        metaJson: { policy_id: id, limit_type: row.limit_type },
      });
