...
function renderLinkedCell(href: string, content: ReactNode, muted = false) {
  return (
    <Link
      className={`block rounded px-2 py-1 -mx-2 -my-1 hover:bg-white/30 dark:hover:bg-white/[0.06] ${muted ? "text-slate-500 dark:text-[#9da5b4]" : "text-slate-800 dark:text-[#e8eaf0]"}`}
      href={href}
    >
      {content}
    </Link>
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {rows.map((row) => {
              ...
              return (
                <tr className="border-t border-slate-200/20 dark:border-white/[0.06] transition-colors hover:bg-white/30 dark:hover:bg-white/[0.06]" key={row.id}>
            ...
            {rows.length === 0 ? (
              <tr>
                <td className="px-3 py-8 text-center text-slate-400 dark:text-[#9da5b4]" colSpan={6}>
